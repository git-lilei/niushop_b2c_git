...
function GetDataList(page) {
	...
	api("NsBargain.Bargain.myBargain", {"page_index": page}, function (res) {
		...
		if (data['data'].length == 0) {
			html += '<p class="empty">您当前还没有砍价哦</p>';
		} else {
			html += '<ul>';
			for (var i = 0; i < data['data'].length; i++) {
				...
				if (curr.status == 1) {
					...
					html += '<a href="#" class="ns-text-color">砍价成功</a>';
				}
				html += '</label>';
				...
				html += '<span class="money ns-text-color">￥' + curr.goods_money + '</span>';
				html += '</div>';
				html += '</article>';
				html += '<footer>';
				if (curr.status == 1) {
					html += '<span>还差<strong class="ns-text-color">' + (curr.goods_money - curr.bargain_money) + '</strong>元，<time data-end-time="' + curr.end_time + '">剩余00:00:00</time></span>';
					html += '<button class="btn-invitation-friend ns-bg-color" data-launch-id="' + curr.launch_id + '">邀请好友砍价</button>';
				} else if (curr.status == 2) {
					html += '<ul>';
					html += '<li>';
					html += '<div>';
