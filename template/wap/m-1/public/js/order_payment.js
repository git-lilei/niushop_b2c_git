...
$(function () {
	...
	$('.distribution-time-out .shipping-time-list li').click(function () {
		...
		var text = '预计' + $(this).text() + ' ' + $('.distribution-time-out .time-out-list span.selected').text() + ' ' + '送達';
	...
	$('.coupon-list li').click(function () {
		if ($(this).hasClass('selected')) {
			...
			vue.couponCont = `<span class="coupon-mark ns-text-color ns-border-color">有` + couponNum + `張券可用</span> <span>去使用</span>`;
		} else {
			$(this).addClass('selected').siblings('li').removeClass('selected');
			...
			vue.couponCont = `已使用1張，可抵扣NT$<span>` + countMoney + `</span>`;
		}
		order.calculate();
	});
...
function Order() {
	...
	this.getValue = function () {
		if (data.is_virtual == 0) {
			...
			var shipping_pick_up = $('#shipping_pick_up').val();
			if (shipping_company > 0) {
				data.shipping_info.shipping_type = 1;
			} else {
				data.shipping_info.shipping_type = 2;
			}
			if (data.shipping_info.shipping_type == 1) {
				data.shipping_info.shipping_company_id = shipping_company;
				...
				data.shipping_info.pick_up_id = shipping_pick_up;
			}
		} else {
		...
		if (this.verify()) {
			...
			api('System.Order.orderCreate', {'data': JSON.stringify(data)}, function (res) {
				if (res.code == 0) {
					...
					fbTouch.TouchPurchase($('.total-price .ns-text-color span').text(), 'TWD');
