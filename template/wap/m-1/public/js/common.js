	...
	share: function (params) {
		...
		api('System.Member.shareContents', params, function (res) {
			...
			wx.ready(function () {
				...
				var desc = share_contents + share_nick_name + "收藏熱度：★★★★★";
...
function FacebookTouchPixel() {
	this.addToCart = 0;
	this.initiateCheckout = 0;
	this.addPaymentInfo = 0;
	...
	this.pageView = 0;
	this.purchase = 0;
	this.completeRegistration = 0;
	this.contact = 0;
	this.search = 0;
	this.getCoupon = 0;
	this.TouchAddToCart = function() {
		if (this.addToCart == 0) {
			fbq('track', "AddToCart");
			this.addToCart++;
		}
	};
	this.TouchInitiateCheckout = function(){
		if (this.initiateCheckout == 0) {
			fbq('track', "InitiateCheckout");
			this.initiateCheckout++;
		}
	};
	this.TouchAddPaymentInfo = function(){
		if (this.addPaymentInfo == 0) {
			fbq('track', "AddPaymentInfo");
			this.addPaymentInfo++;
		}
	};
	this.TouchAddToWishlist = function(){
		if (this.addToWishlist == 0) {
			fbq('track', "AddToWishlist");
			this.addToWishlist++;
		}
	};
	this.TouchLead = function(){
		if (this.lead == 0) {
			fbq('track', "Lead");
			this.lead++;
		}
	};
	this.TouchSubmitApplication = function(){
		if (this.submitApplication == 0) {
			fbq('track', "SubmitApplication");
			this.submitApplication++;
		}
	};
	this.TouchViewContent = function(){
		if (this.viewContent == 0) {
			fbq('track', "ViewContent");
			this.viewContent++;
		}
	};
	this.TouchPageView = function(){
		if (this.pageView == 0) {
			fbq('track', "PageView");
			this.pageView++;
		}
	};
	this.TouchPurchase = function(value, currency){
		if (this.purchase == 0) {
			fbq('track', "Purchase", {'value': value, 'currency': currency});
			this.purchase++;
		}
	};
	this.TouchCompleteRegistration = function(){
		if (this.completeRegistration == 0) {
			fbq('track', "CompleteRegistration");
			this.completeRegistration++;
		}
	};
	this.TouchContact = function(){
		if (this.contact == 0) {
			fbq('track', "Contact");
			this.contact++;
		}
	};
	this.TouchSearch = function(){
		if (this.search == 0) {
			fbq('track', "Search");
			this.search++;
		}
	};
	this.TouchGetCoupon = function(){
		if (this.getCoupon == 0) {
			fbq('track', "GetCoupon");
			this.getCoupon++;
		}
...
fbTouch = new FacebookTouchPixel();