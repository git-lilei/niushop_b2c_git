...
function GetDataList(page) {
	...
	api("NsPintuan.Pintuan.pintuanOrder", {"page": page}, function (res) {
		...
		if (data['data'].length == 0) {
			html += '<p style="padding: 50px;background: #ffffff;text-align: center;">您當前還沒有拼單哦</p>';
		} else {
			html += '<ul>';
			for (var i = 0; i < data['data'].length; i++) {
				...
				html += '<header>';
				html += '<label>';
				html += '<span>發起了拼單</span>';
				
				if (curr.status == 1) {
					html += '<a class="ns-text-color" href="#">待分享</a>';
				} else if (curr.status == 2) {
					html += '<a class="ns-text-color" href="#">拼單成功</a>';
				} else if (curr.status == -1) {
					html += '<a class="ns-text-color" href="#">拼單失敗</a>';
				}
				html += '</label>';
				...
				html += '<span class="money">NT$' + curr.tuangou_money + '<span class="num ns-text-color-gray">' + curr.tuangou_num + '人拼單</span></span>';
				html += '</div>';
				html += '</article>';
				
				html += '<footer>';
				if (curr.status == 1) {
					html += '<span>還剩<strong class="ns-text-color">' + (curr.tuangou_num - curr.real_num) + '</strong>人，<time data-end-time="' + curr.end_time + '">剩余00:00:00</time></span>';
					html += '<button class="btn-invitation-friend ns-bg-color" data-goods-id="' + curr.goods_id + '" data-group-id="' + curr.group_id + '">邀請好友拼單</button>';
				} else if (curr.status == 2) {
					html += '<ul>';
					html += '<li>';
					html += '<div>';
					...
					html += '</li>';
					html += '</ul>';
					html += '<button class="btn-order-info ns-border-color-gray" onclick="location.href=\'' + __URL(APPMAIN + '/order/detail?order_id=' + curr.order_id) + '\'">查看訂單詳情</button>';
				}
				html += '</footer>';
				html += '</li>';
			}
