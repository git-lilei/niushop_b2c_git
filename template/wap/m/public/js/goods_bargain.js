    ...
    api('NsBargain.Bargain.bargainList', {"page_index":page}, function(res){
        if(res.code == 0){
            ...
            if(data['data'].length==0){
                html += '<div class="empty"><img src="'+WAPIMG+'/wap_nodata.png"/><p class="ns-text-color-gray">沒有找到您想要的砍價商品…</p></div>';
            }else{
                html += '<ul>';
                    ...
                    html += `<li onclick="location.href='`+ __URL(APPMAIN + '/goods/detail?goods_id=' + curr.goods_id + '&bargain_id=' + curr.bargain_id) +`'">
                        ...
                        <footer>
                            ...
                            <div class="assemble-tag">
                                <div class="already-num ns-text-color ns-bg-color-fadeout-80">砍價</div>`;
                                if(curr.shipping_fee == 0){
                                    html += '<div class="people-num ns-text-color ns-border-color-fadeout-80">包郵</div>';
                                }
                        html += `</div>
                        <div class="assemble-foot">
                            <div class="assemble-foot-left">
                                 <div class="tuangou-money ns-text-color">NT$`+ curr.promotion_price +`</div>
                            </div>
                           <div class="assemble-foot-right">
                                <div class="mui-btn-danger primary">發起砍價</div>
                            </div>
                        </div>
                    </footer>
                </li>`;
