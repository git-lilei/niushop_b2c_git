...
(function (factory) {
  ...
  //extend_css 扩展自定义样式
  //moveCallBack	拖拽时触发回调事件
  //endCallBack 拖拽结束后触发回调事件
  $.fn.arrangeable = function(options,extend_css,moveCallBack,endCallBack) {
    var dragging = false;
    var $clone;
    var dragElement;
    ...
    function dragMoveHandler(e) {
      ...
      // check for drag threshold (drag has not started yet)
        ...
        if($.isFunction(moveCallBack)) moveCallBack();
    ...
    function dragEndHandler(e) {
      if (dragging) {
        ...
        if($.isFunction(endCallBack)) endCallBack();
      }
