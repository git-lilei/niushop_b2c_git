    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,success: function(res){
          typeof options.renderResponse === 'function' && (res = options.renderResponse(res));//非官方
...
/*    if(data.length === 0){
    ...
    if(data.length === 0){
    	that.layMain.find('.layui-table').css('width', '100%');
    	that.layMain.find('.'+ NONE).remove();
    	that.layMain.find('tbody').html('<div class="'+ NONE +'">'+ options.text.none +'</div>');
    }
  };
  ...
  Class.prototype.syncCheckAll = function(){
    ...
    ,pageCheckAll = that.layPage.find('input[name="layTableCheckbox"]')
    ...
    //分页中的全选按钮
    if(!pageCheckAll[0]) return;

    if(table.checkStatus(that.key).isAll){
      if(!checkAllElem[0].checked){
        checkAllElem.prop('checked', true);
        pageCheckAll[0].checked = true;
  ...
  Class.prototype.events = function(){
    ...
    that.elem.on('click', 'input[name="layTableCheckbox"]+', function(){
      ...
      ,pageChild = that.layPage.find('input[name="layTableCheckbox"]')
      ...
      if(isAll){
        ...
        pageChild.prop('checked',checked);
        that.syncCheckAll();
        that.renderForm('checkbox');
      } else {
    ...
    //批量操作事件
    that.layPage.on('click', '.layui-laypage-batch-operation *[lay-event]', function(){
        ...
        layui.event.call(this, MOD_NAME, 'batchOperation('+ filter +')',{
