...
function PackageProductInfo() {
    ...
    obj.cards =  $('.sku_type_1 [name="stock_data"]').attr('card_data');
	return obj;
}
 ...
 * 卡卷库存层
 */
function cardStockDialog(event){
    ...
    $('#card_data').val(obj.attr('card_data').replace(reg,'\n'));
...
function ajaxAddVirtualGoods(){
	var card_text = $('#card_data').val();
    var card_arr = card_text.split("\n");
    
    card_arr = $.unique(card_arr);
    card_arr = card_arr.filter(s => $.trim(s).length > 0).reverse();

    obj = card_obj.parent();

    obj.attr('card_data', card_arr.toString());
	
    var default_stock = obj.children('input').attr('default');
    var stock = Number(default_stock) + Number(card_arr.length);
    obj.children('a').text(`库存管理【${stock}】`);
    obj.children('input').val(stock);
    stockToUpdate();
    $('#add_virtual_goods').modal('hide');
}

function stockToUpdate(){
    var stock = 0;
    if($('[name="sku_type"]:checked').val() == 0){
        $('#txtProductCount').val($('#stock').val());
    }else{
        $('.table-class [name="stock"]').each(function(){
            stock = Number(stock) + Number($(this).val());
        })
        $('#txtProductCount').val(stock);
    }
}