...
class Ueditor extends BaseController{
    public function index(){
        ...
        $CONFIG = json_decode(preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents("./template/admin/public/js/ueditor/php/config.json")), true);
        $action = $_GET['action'];
    ...
    private function upImage($fieldName){
        ...
        if(!empty($_FILES[$fieldName])){//上传成功
            $info = $upload_service->image($_FILES[$fieldName], $upload_path);
            if($info['code'] > 0){
                $data=array(
                    'state' => 'SUCCESS',
                    'url' => $info['data']['path'],
                    'title' => $info['data']['file_name'],
                    'original' => $info['data']['new_name'],
                    'type' => '.' . $info['data']['file_ext'],
                    'size' => $info['data']['size']
                );
            }else{
    ...
    private function saveRemote($config,$fieldName){
        ...
        $dirname = UPLOAD . '/ueditor/image/'.date('Ymd');
        $file['oriName'] = $m ? $m[1] : "";
    ...
    private function upBase64($config,$fieldName){
        ...
        $dirname = UPLOAD . '/ueditor/scrawl/'.date('Ymd');
        $file['filesize'] = strlen($img);
