...
class WchatOauth
{
	...
	public function tmplmsg($param)
    {
        ...
        $json = json_encode($array);
        $json_url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=%s";
        return $this->get_url_return($json_url, $json);
    }
	 ...
	 * @return string|\data\extend\unknown
	 */
	 ...
	 * 小程序获取token
	 */
	private function applet_get_access_token()
	{
		$config = new Config();
		$applet_config = $config->getInstanceAppletConfig(0);
		if (empty($applet_config['value']['appid']) || empty($applet_config['value']['appsecret'])) {
		    return -50;
        }
		$url = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=' . $applet_config['value']['appid'] . '&secret=' . $applet_config['value']['appsecret'];
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		...
		if ($strjson == false || empty($strjson)) {
			...
			if (empty($token)) {
				...
				// 注意如果是多用户需要
				cache::set('applet-token-' . $this->author_appid, $token, 3600);
			}
			return $token;
		}
	 ...
	 * @return string|unknown
	 */
	private function applet_get_url_return($url, $data = '', $needToken = false)
	{
		// 第一次为空，则从文件中读取
		if (empty($this->applet_token)) {
			$this->applet_token = cache::get('applet-token-' . $this->author_appid);
		}
		// 为空则重新取值
		if (empty($this->applet_token) or $this->applet_token == -50 or $needToken) {

			$token = $this->applet_get_access_token();
			if ($token == -50) {
			    return -50;
            }
			$this->applet_token = $token;
		}
		$newurl = sprintf($url, $this->applet_token);
		$curl = curl_init(); // 创建一个新url资源
		curl_setopt($curl, CURLOPT_URL, $newurl);
		curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, FALSE);
		curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, FALSE);
		...
		if (!empty($strjson->errcode)) {
			switch ($strjson->errcode) {
				case 40001:
					return $this->applet_get_url_return($url, $data, true); // 获取access_token时AppSecret错误，或者access_token无效
					break;
				case 40014:
					return $this->applet_get_url_return($url, $data, true); // 不合法的access_token
					break;
				case 42001:
					return $this->applet_get_url_return($url, $data, true); // access_token超时
					break;
				case 45009:
					return json_encode(array(
						"errcode" => -10,
     ...
     * @param $data
     */
	public function getAppletQrcode($scene, $page, $auto_color = true, $file_path = '', $file_name = '')
    {
        $cache_param = 'applet_qrcode_' . $scene . '_' . $page . '_' . $auto_color . '_' . $file_path . '_' . $file_name;
        $cache = Cache::get($cache_param);
        if (empty($cache) || ! file_exists($cache)) {
            ...
            $path = $this->applet_get_url_return($url, json_encode($data));
            if (strlen($path) > 1000) {
                $file_path = empty($file_path) ? UPLOAD . '/qrcode/applet_qrcode' : $file_path;
                // 检测文件夹是否存在，不存在则创建文件夹
                if (!is_dir($file_path)) {
                    $mode = intval('0777', 8);
                ...
                $file_name = empty($file_name) ? md5(rand(0, 1000) . time() . rand(0, 1000)) : $file_name;
