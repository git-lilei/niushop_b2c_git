...
class GoodsMansong extends BaseService
{
	...
	public function getGoodsMansongName($goods_id)
	{
		...
		if (!empty($mansong_detail)) {
			foreach ($mansong_detail['rule'] as $k => $v) {
				$mansong_name .= '滿' . $v['price'] . '減' . $v['discount'] . ' ';
				if ($v['free_shipping'] == 1) {
					$mansong_name .= '免郵' . ' ';
				}
				if ($v['give_point'] != 0) {
					$mansong_name .= '贈送' . $v['give_point'] . '積分' . ' ';
				}
				if ($v['give_coupon'] != 0) {
					...
					$mansong_name .= '贈送' . $coupon_name['money'] . '優惠卷' . ' ';
				}
				if ($v['gift_id'] != 0) {
					...
					$mansong_name .= '贈送' . $gift_name['gift_name'];
				}
				$mansong_name .= '; ';
			}
		}
	 ...
	 * 查询对应满减送活动的商品列表的优惠情况
	 *
	 * @param array $mansong_obj (只针对部分商品满减)
